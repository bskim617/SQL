...
#실습2-19 t_dist <> 3 3과 다르다, 같지 않다로 해도 됨
select * from `bank_transaction` where `t_dist` = 1 or `t_dist` = 2 order by `t_dist`,`t_amount` desc;

#실습2-20 (t_dist가 1이면 ,2이면, 3이면) 논리적으로 따라서 if로 if(t_dist = 1, 1,null) >> 문법
select 
...
#실습2-21 t_dist가 1 이면 입금 2이면 출금 3이면 조회 >> 논리적 표현  (case ~ when ~ then (else) end as '') 문법
select `t_dist`,
case
